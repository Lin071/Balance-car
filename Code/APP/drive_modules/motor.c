...
**************************************************************************/
static void AMotor_PWM_Init(uint16_t arr,uint16_t psc)
{  
...
//arr：自动重装值
//psc：时钟预分频数
//TIM3_PWM_Init(7199,0);//PWM频率=72000/(7199+1)=10Khz
**************************************************************************/
static void BMotor_PWM_Init(uint16_t arr,uint16_t psc)
{  
	GPIO_InitTypeDef GPIO_InitStructure;
	TIM_TimeBaseInitTypeDef  TIM_TimeBaseStructure;
	TIM_OCInitTypeDef  TIM_OCInitStructure;
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_0; 			// TIM3_3
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;  	//复用推挽输出
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOB, &GPIO_InitStructure);
...
{  	
	//对输出进行限制，防止占空比超过100%
	motoA = motoA > 7100 ? 7100 : motoA < -7100 ? -7100 : motoA;
	motoB = motoB > 7100 ? 7100 : motoB < -7100 ? -7100 : motoB;
	...
	// 输出PWM
	TIM_SetCompare4(TIM3, motoA < 0 ? -motoA : motoA);
    TIM_SetCompare3(TIM3, motoB < 0 ? -motoB : motoB);
}
