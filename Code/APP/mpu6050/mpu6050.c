...
/**************************************************************************
函数功能：起到中断唤醒的作用：当MPU6050将数据准备好了，会通过中断信号来通知MCU读取数据
         5ms定时中断由MPU6050的INT引脚触发，严格保证采样和数据处理的时间同步	
设   置: 在MPU6050的采样频率设置中，设置成100HZ，即可保证6050的数据是10ms更新一次
...
void EXTI9_5_IRQHandler(void) 
{
  if(PBin(5)==0)		//判断是否MUP6050发生的中断
  {
    ...
    //通知直立环控制任务读取MPU6050数据并进行直立控制
    vTaskNotifyGiveFromISR(xBalanceTaskHandle, NULL);   // 通知xBalanceTaskHandle任务，并且不设置pxHigherPriorityTaskWoken的值
  }
}
...
void Mpu6050_Init(void)
{
    ...
    {
        OLED_ShowString(0, 8, (u8 *)"MPU init...", 16);
        delay_ms(200);
    }
    delay_ms(200);
    while(mpu_dmp_init())		//=====MPU6050内置DMP初始化，用于获取姿态角
    {
        OLED_ShowString(0, 8, (u8 *)"DMP init...", 16);
        delay_ms(200);
    }
}
